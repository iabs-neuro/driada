#!/usr/bin/env python
"""
Generate Notebook 05: Advanced Capabilities
=============================================
...
Source examples:
  1. examples/functional_organization/functional_organization.py
  2. examples/loo_dr_analysis/loo_dr_analysis.py
  3. examples/rsa/rsa_example.py
  4. examples/rnn_activations/rnn_activations.py
"""
...
# ===== SECTION 1: EMBEDDING SELECTIVITY ====================================
...
"\n"
"> **Note:** PCA and UMAP are used here as default latent extractors because\n"
"> they are fast and unsupervised. For richer representations, consider\n"
"> disentangling methods (e.g. Beta-VAE via\n"
"> [`flexible_ae`](https://driada.readthedocs.io/en/latest/api/dim_reduction/neural_methods.html))\n"
"> or behavior-aware embeddings (e.g. [CEBRA](https://cebra.ai/)). The\n"
"> workflow below applies to any embedding that produces an\n"
"> `(n_frames, n_components)` array.\n"
"\n"
"Key APIs:\n"
...
# ===== SECTION 2: LOO NEURON IMPORTANCE ====================================

cells.append(md_cell(
"## 2. Leave-one-out neuron importance\n"
"\n"
"How much does each neuron contribute to the population manifold? We\n"
"remove neurons one at a time, recompute the embedding, and measure the\n"
"degradation using three ground-truth metrics:\n"
"\n"
"- [`compute_reconstruction_error`](https://driada.readthedocs.io/en/latest/api/dim_reduction/manifold_metrics.html#driada.dim_reduction.manifold_metrics.compute_reconstruction_error) -- L2 distance between the embedding and the true variable (here, head direction angles)\n"
"- [`compute_decoding_accuracy`](https://driada.readthedocs.io/en/latest/api/dim_reduction/manifold_metrics.html#driada.dim_reduction.manifold_metrics.compute_decoding_accuracy) -- R² of a linear decoder from embedding to variable\n"
"- [`compute_embedding_alignment_metrics`](https://driada.readthedocs.io/en/latest/api/dim_reduction/manifold_metrics.html#driada.dim_reduction.manifold_metrics.compute_embedding_alignment_metrics) -- Spearman correlation between embedding distances and variable distances\n"
"\n"
"We then compare LOO importance against INTENSE selectivity\n"
...
"The hypothesis is that neurons important for the manifold are also the\n"
"ones encoding the behavioral variable. This is plausible but **not\n"
"guaranteed** -- LOO measures structural importance (manifold degradation)\n"
"while INTENSE measures statistical dependence (mutual information). A\n"
"positive correlation provides convergent evidence linking single-neuron\n"
"selectivity to population geometry."
))
...
cells.append(md_cell(
"### Step 1: Baseline embedding\n"
"\n"
"We first compute a full-population embedding with Isomap and evaluate it\n"
"against ground truth head direction angles. This baseline is the reference\n"
"that each LOO embedding will be compared against."
))
...
"ds_loo = 5\n"
"\n"
"print('[2/4] Computing baseline embedding...')\n"
"\n"
"neural_data_loo = exp_loo.calcium.scdata  # (n_neurons, n_timepoints)\n"
"n_neurons_loo = neural_data_loo.shape[0]\n"
"\n"
"# Ground truth: head direction angles (downsampled to match embedding)\n"
...
"    \"\"\"Compute embedding and align ground truth (filtering lost nodes).\"\"\"\n"
...
"    \"\"\"Compute all three ground-truth reconstruction metrics.\"\"\"\n"
"    metrics = {}\n"
"    try:\n"
...
"\n"
"print(f'  Baseline metrics (all {n_neurons_loo} neurons):')\n"
"print(f'    alignment_corr:      {baseline_metrics[\"alignment_corr\"]:.4f}')\n"
"print(f'    decoding_accuracy:   {baseline_metrics[\"decoding_accuracy\"]:.4f}')\n"
"print(f'    reconstruction_error: {baseline_metrics[\"reconstruction_error\"]:.4f}')"
))

cells.append(md_cell(
"### Step 2: Leave-one-out loop\n"
"\n"
"For each neuron, we remove it from the population, recompute the Isomap\n"
"embedding, and measure the three metrics again. The degradation relative\n"
"to baseline tells us how important that neuron was for the manifold."
))

cells.append(code_cell(
"print('  LOO analysis for', n_neurons_loo, 'neurons...')\n"
"\n"
...
"print(f'  Done. {loo_results.iloc[1:].notna().all(axis=1).sum()} neurons computed successfully.')"
))

cells.append(md_cell(
"### Step 3: Importance scoring\n"
"\n"
"Each neuron's importance is the average normalized degradation across the\n"
"three metrics. Higher values mean the neuron contributed more to the\n"
"embedding quality."
))
...
"    print('Top 5 most important neurons:')\n"
...
"print('  Hypothesis: neurons important for the manifold are also selective')\n"
"print('  for the encoded variable. A positive Spearman correlation supports')\n"
"print('  this, but the two measures capture different aspects.')\n"
"\n"
...
"    print(f'\\n  Spearman correlation: r = {corr:.3f}, p = {pval_corr:.3e}')\n"
"    if pval_corr < 0.05 and corr > 0:\n"
"        print('  -> Positive correlation: neurons important for the manifold')\n"
"        print('     tend to be the ones selective for head_direction.')\n"
"        print('     This supports (but does not prove) the hypothesis.')\n"
"    elif pval_corr < 0.05:\n"
"        print('  -> Negative correlation (unexpected -- see discussion above).')\n"
"    else:\n"
"        print('  -> No significant correlation found. LOO importance and')\n"
"        print('     INTENSE MI may capture different aspects of encoding.')\n"
"\n"
...
"ax.set_title(f'Convergent evidence\\nr={corr:.3f}, p={pval_corr:.2e}')\n"
"ax.grid(True, alpha=0.3)\n"
"\n"
...
"\n"
"A **Representational Dissimilarity Matrix** (RDM) summarizes how a\n"
"population represents different stimuli or conditions. Each entry is\n"
"the pairwise dissimilarity between the population responses to two\n"
"conditions (e.g. correlation distance between the mean firing-rate\n"
"vectors). Because the RDM abstracts away neuron identity and count,\n"
"you can directly compare representations across:\n"
"\n"
"- **Brain regions** with different neuron numbers\n"
"- **Recording sessions** with different noise realizations\n"
"- **Computational models** and neural data\n"
"\n"
"DRIADA's [`rsa`](https://driada.readthedocs.io/en/latest/api/rsa.html) module provides:\n"
"\n"
"**Core computation:**\n"
"- [`compute_rdm_unified`](https://driada.readthedocs.io/en/latest/api/rsa/core.html#driada.rsa.core.compute_rdm_unified) -- RDM from neural data + condition labels (auto-dispatches for arrays, MVData, or Experiment)\n"
"- [`compute_rdm`](https://driada.readthedocs.io/en/latest/api/rsa/core.html#driada.rsa.core.compute_rdm) -- RDM from a (n_items x n_features) pattern matrix\n"
...
"- [`compare_rdms`](https://driada.readthedocs.io/en/latest/api/rsa/core.html#driada.rsa.core.compare_rdms) -- correlate two pre-computed RDMs\n"
...
"\n"
"**Integration with DRIADA objects:**\n"
"- [`compute_experiment_rdm`](https://driada.readthedocs.io/en/latest/api/rsa/integration.html#driada.rsa.integration.compute_experiment_rdm) -- RDM from an `Experiment`\n"
"- [`compute_mvdata_rdm`](https://driada.readthedocs.io/en/latest/api/rsa/integration.html#driada.rsa.integration.compute_mvdata_rdm) -- RDM from `MVData` (bridges DR -> RSA)\n"
"- [`rsa_between_experiments`](https://driada.readthedocs.io/en/latest/api/rsa/integration.html#driada.rsa.integration.rsa_between_experiments) -- cross-experiment comparison\n"
"\n"
"**Visualization:** [`plot_rdm`](https://driada.readthedocs.io/en/latest/api/rsa/visualization.html#driada.rsa.visual.plot_rdm), [`plot_rdm_comparison`](https://driada.readthedocs.io/en/latest/api/rsa/visualization.html#driada.rsa.visual.plot_rdm_comparison)"
))
...
cells.append(md_cell(
"### 3.4 MVData integration -- bridging DR and RSA\n"
"\n"
"RSA works seamlessly with\n"
"[`MVData`](https://driada.readthedocs.io/en/latest/api/dim_reduction/data_structures.html#driada.dim_reduction.data.MVData)\n"
"objects from the DR pipeline. This lets you compare representational\n"
"structure in the *embedding space* rather than the raw neural space --\n"
"for example, testing whether PCA and UMAP preserve the same\n"
"inter-condition distances.\n"
"\n"
"Pass an `MVData` object (or its `.data` array) directly to\n"
"[`compute_rdm_unified`](https://driada.readthedocs.io/en/latest/api/rsa/core.html#driada.rsa.core.compute_rdm_unified).\n"
"Condition labels map timepoints to experimental items."
))
...
"    'w_in': 0.8,\n"
"    'noise_sigma': 0.05,\n"
"    'duration': 600,  # shorter durations may yield zero selective units\n"
"    'fps': 20,\n"
"    'tau_smooth': 1.0,\n"
"    'obs_noise': 0.01,\n"
"    'n_shuffles_stage1': 100,\n"
...
"\n"
"if len(significant_neurons_rnn) == 0:\n"
"    print('\\n  No selective units detected. With random fixed weights, RNN')\n"
"    print('  units may not develop stable input selectivity. Try increasing')\n"
"    print('  duration or relaxing pval_thr.')"
))
...
out_path = os.path.join(out_dir, "05_advanced_capabilities.ipynb")
