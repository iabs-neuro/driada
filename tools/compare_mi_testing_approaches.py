...
def analyze_testing_approaches():
    ...
    for i, data in enumerate(shuffle_data):
        ...
        print(
            f"  Shuffle distribution: mean={data.statistical_properties['mean']:.4f}, "
        ...
        comparison_results.append({"data": data, "results": results})
    ...
    for method1, method2, count, total in disagreements:
        if count > 0:
            print(
                f"  {method1} vs {method2}: {count}/{total} cases disagree ({100*count/total:.1f}%)"
            )
    ...
    for i, data in enumerate(shuffle_data[:3]):  # Test on first 3 cases
        p_val, details = tester.compute_p_value(
            data.true_mi, data.shuffle_values, return_details=True
        )
        print(f"\nCase {i+1}:")
...
def evaluate_performance(comparison_results):
    ...
    for method in methods:
        ...
        if total > 0:
            ...
            performance[method] = {"sensitivity": 0, "specificity": 0, "accuracy": 0}
...
def create_comparison_visualizations(comparison_results):
    ...
    if comparison_results:
        ...
        counts, bins, _ = ax4.hist(
            shuffle_vals, bins=30, density=True, alpha=0.5, color="gray"
        )
        ...
        ax4.plot(
            x_range, stats.norm.pdf(x_range, *norm_params), "b-", label="Normal fit"
        )
        ...
        if np.all(shuffle_vals > 0):
            ...
            ax4.plot(
                x_range,
                stats.gamma.pdf(x_range, *gamma_params),
                "r-",
                label="Gamma fit",
            )
