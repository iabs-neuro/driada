...
class IsolatedCoverageRunner:
    ...
    def run_single_test_file(
        self, test_file: Path, module: Optional[str] = None
    ) -> Dict[str, any]:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if coverage_json.exists():
                try:
                    ...
                    if "totals" in cov_data:
                        test_result["coverage"]["TOTAL"] = cov_data["totals"][
                            "percent_covered"
                        ]
            ...
            if not test_result["coverage"] and result.stdout:
                test_result["coverage"] = self._parse_text_coverage(
    ...
    def run_all_modules(self) -> Dict[str, any]:
        ...
        module_dirs = [
            d for d in unit_dir.iterdir() if d.is_dir() and not d.name.startswith("__")
        ]
...
def main():
    ...
    print(
        "This tool runs each test file in isolation to avoid torch import conflicts\n"
    )
