    ...
    for neuron in exp.neurons:
        if not hasattr(neuron, "sp"):
            ...
            spike_times = np.random.choice(
                T_actual, size=int(T_actual * 0.01), replace=False
            )
...
def test_plot_neuron_feature_pair_no_density(mixed_features_experiment):
    ...
    # Should run without error
    fig = plot_neuron_feature_pair(
        exp, 0, "speed", ind1=0, ind2=100, add_density_plot=False
    )
    assert fig is not None
    plt.close("all")
...
def test_plot_disentanglement_heatmap():
    ...
    disent_matrix = np.array(
        [[0, 10, 20, 5], [20, 0, 15, 25], [5, 15, 0, 10], [25, 5, 20, 0]]
    )
    count_matrix = np.array(
        [[0, 20, 30, 10], [20, 0, 30, 50], [30, 30, 0, 20], [10, 50, 20, 0]]
    )
    ...
    custom_cmap = LinearSegmentedColormap.from_list(
        "custom", [(0, 0, 1), (1, 1, 1), (1, 0, 0)]
    )
    fig, ax = plot_disentanglement_heatmap(
        disent_matrix, count_matrix, feat_names, cmap=custom_cmap
    )
    assert fig is not None
    plt.close("all")
...
def test_plot_disentanglement_summary():
    ...
    fig = plot_disentanglement_summary(
        disent_matrices, count_matrices, feat_names, title_prefix="Multi-Exp: "
    )
    assert fig is not None
    plt.close("all")
    ...
    fig = plot_disentanglement_summary(
        disent_matrices, count_matrices, feat_names, experiments=["Exp1", "Exp2"]
    )
    assert fig is not None
    plt.close("all")
...
def test_plot_disentanglement_summary_with_nan():
    ...
    disent_matrix = np.array(
        [[0, 10, 0, 5], [20, 0, 15, 0], [0, 15, 0, 10], [25, 0, 20, 0]]
    )
    count_matrix = np.array(
        [[0, 20, 0, 10], [20, 0, 30, 0], [0, 30, 0, 20], [10, 0, 20, 0]]
    )
    feat_names = ["feat1", "feat2", "feat3", "feat4"]
...
def test_plot_selectivity_heatmap():
    ...
    for feat_name in ["d_feat_0", "d_feat_1", "c_feat_0", "c_feat_1"]:
        ...
        for neuron_id in range(10):
            ...
            if (
                (neuron_id < 3 and feat_name == "d_feat_0")
                or (neuron_id >= 7 and feat_name == "c_feat_0")
                or (neuron_id == 5 and feat_name in ["d_feat_1", "c_feat_1"])
            ):
    ...
    assert all(
        v > 0 for v in stats["metric_values"]
    )  # All MI values should be positive
    plt.close("all")
    ...
    fig, ax, stats = plot_selectivity_heatmap(
        exp, significant_neurons, significance_threshold=0.01
    )
    assert fig is not None
    plt.close("all")
    ...
    fig2, ax2, stats2 = plot_selectivity_heatmap(exp, significant_neurons, ax=ax_custom)
    ...
    fig, ax, stats = plot_selectivity_heatmap(
        exp, significant_neurons, vmin=0.1, vmax=0.6
    )
    assert fig is not None
    plt.close("all")
...
def test_plot_selectivity_heatmap_edge_cases():
    ...
    fig, ax, stats = plot_selectivity_heatmap(
        exp, significant_neurons, significance_threshold=0.05
    )
    assert fig is not None
    ...
    fig, ax, stats = plot_selectivity_heatmap(exp, {0: ["d_feat_0"]}, metric="corr")
    assert fig is not None
