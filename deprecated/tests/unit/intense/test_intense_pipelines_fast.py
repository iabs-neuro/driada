...
class TestIntensePipelinesFast:
    ...
    def test_compute_cell_feat_significance_mocked(
        self, mock_mi_sig, mock_info, mock_experiment
    ):
    ...
    @patch("driada.intense.pipelines.compute_info_parallel")
    @patch("driada.intense.pipelines.compute_mi_significance_parallel")
    def test_compute_feat_feat_significance_mocked(
        self, mock_mi_sig, mock_info, mock_experiment
    ):
    ...
    @patch("driada.intense.pipelines.compute_info_parallel")
    @patch("driada.intense.pipelines.compute_mi_significance_parallel")
    def test_compute_cell_cell_significance_mocked(
        self, mock_mi_sig, mock_info, mock_experiment
    ):
    ...
    def test_performance_with_mocking(self, mock_experiment):
        ...
        with (
            patch("driada.intense.pipelines.compute_info_parallel") as mock_info,
            patch(
                "driada.intense.pipelines.compute_mi_significance_parallel"
            ) as mock_mi,
        ):
            ...
            start = time.time()
            compute_feat_feat_significance(
                mock_experiment, mode="stage1", n_shuffles_stage1=5, verbose=False
            )
            duration = time.time() - start
...
class TestIntegrationScenariosFast:
    ...
    def test_mixed_selectivity_scenario_mocked(self, mock_mi, mock_info, mock_gen):
        ...
        stats, sig, info, results = compute_cell_feat_significance(
            exp, mode="stage1", n_shuffles_stage1=5, verbose=False
        )
    ...
    def test_multiselectivity_detection_mocked(self, mock_patterns):
        ...
        patterns = mock_patterns(
            num_neurons=5, feature_names=["feat1", "feat2"], mixed_fraction=0.2
        )
