...
def plot_embedding_comparison(
    ...
    features : dict, optional
        ...
        Default features used: 'angle' (circular position) and 'speed'
    ...
    for i, method in enumerate(methods):
        ...
        if features is not None and 'angle' in features:
            ...
            scatter = ax1.scatter(
                ...
                c=angle_norm, cmap='hsv', alpha=0.7, s=2,
        ...
        if features is not None and 'speed' in features:
            ...
            scatter = ax2.scatter(
                embedding[:, 0], embedding[:, 1],
                c=speed, cmap='viridis', alpha=0.7, s=2,
                edgecolors='none'
            )
...
def plot_trajectories(
    ...
    for i, method in enumerate(methods):
        ...
        ax.scatter(
            embedding[0, 0], embedding[0, 1],
            c='green', s=default_kwargs['marker_size'],
            marker=default_kwargs['start_marker'],
            edgecolors='black', linewidth=2, label='Start', zorder=5
        )
        ax.scatter(
            embedding[-1, 0], embedding[-1, 1],
            c='red', s=default_kwargs['marker_size'],
...
def plot_component_interpretation(
    ...
    for idx, method in enumerate(methods):
        ...
        im = ax.imshow(
            mi_subset, aspect='auto', cmap='YlOrRd',
            vmin=0, vmax=max_mi
        )
    ...
    plt.suptitle('Component Interpretation: Mutual Information between Components and Features', fontsize=16)
    plt.tight_layout()
...
def plot_embeddings_grid(
    ...
    embeddings : dict of dict
        Nested dictionary: {method: {scenario: embedding_array}}
...
def plot_neuron_selectivity_summary(
    ...
    ax.text(0.02, 0.98, f'Total neurons: {total_neurons}',
...
def plot_component_selectivity_heatmap(
    ...
    for ax, method in zip(axes, methods):
        ...
        # Plot heatmap
        im = ax.imshow(method_matrix.T, aspect='auto', cmap='hot',
