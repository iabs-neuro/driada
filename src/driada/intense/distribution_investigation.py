...
from .stats import get_mi_distr_pvalue, get_distribution_function
...
class MIDistributionInvestigator:
    ...
    def generate_test_data(self, 
                         n_scenarios: int = 5,
                         n_shuffles: int = 1000) -> List[ShuffleDistributionData]:
        """
    ...
    def _extract_shuffle_distributions(self, 
                                     exp, 
                                     scenario_name: str,
                                     n_shuffles: int) -> List[ShuffleDistributionData]:
        """
        Extract shuffle distributions from INTENSE analysis.
        ...
        for i, cell_id in enumerate(cell_ids):
            for j, feature_id in enumerate(available_features):
                try:
                    ...
                    ts_feature = feature_ts if hasattr(feature_ts, 'data') else TimeSeries(feature_ts)
                    ...
                    print(f"    Warning: Failed to extract data for neuron {cell_id}, feature {feature_id}: {e}")
                    continue
    ...
    def fit_distributions(self, 
                         data: np.ndarray,
                         distributions: Optional[List[str]] = None) -> Dict[str, DistributionFitResult]:
        """
    ...
    def compare_detection_performance(self) -> Dict[str, Dict[str, float]]:
        ...
        for dist_name in ['norm', 'gamma', 'lognorm']:
            ...
            for data in self.shuffle_data:
                ...
                p_val = get_mi_distr_pvalue(data.shuffle_values, data.true_mi, dist_name)
    ...
    def generate_summary_report(self) -> str:
        ...
        # Data summary
        report.append(f"\nDATA SUMMARY:")
        report.append(f"  Total shuffle distributions analyzed: {len(self.shuffle_data)}")
        ...
        report.append(f"  Non-significant pairs: {len(self.shuffle_data) - significant_count}")
        ...
        avg_stats = {}
        for prop in ['mean', 'std', 'skewness', 'kurtosis', 'shapiro_pvalue', 'normaltest_pvalue']:
            values = [d.statistical_properties[prop] for d in self.shuffle_data if not np.isnan(d.statistical_properties[prop])]
            if values:
        ...
        report.append(f"  Average Shapiro p-value: {avg_stats.get('shapiro_pvalue', 'N/A'):.3f}")
        report.append(f"  Average normaltest p-value: {avg_stats.get('normaltest_pvalue', 'N/A'):.3f}")
        ...
        norm_better_count = sum(1 for d in self.shuffle_data if d.p_value_norm < d.p_value_gamma)
        report.append(f"  Cases where norm gives lower p-value: {norm_better_count}/{len(self.shuffle_data)}")
        
        # Statistical significance of normality
        normal_like_count = sum(1 for d in self.shuffle_data 
                              if d.statistical_properties['shapiro_pvalue'] > 0.05)
        report.append(f"  Distributions that appear normal (Shapiro p>0.05): {normal_like_count}/{len(self.shuffle_data)}")
    ...
    def create_visualizations(self, save_path: Optional[str] = None) -> None:
        ...
        ax1 = axes[0, 0]
        skewness_values = [d.statistical_properties['skewness'] for d in self.shuffle_data]
        ...
        ax2 = axes[0, 1]
        kurtosis_values = [d.statistical_properties['kurtosis'] for d in self.shuffle_data]
        ...
        if self.shuffle_data:
            ...
            ax4.hist(example_data.shuffle_values, bins=30, alpha=0.7, density=True, color='lightcoral')
            ax4.axvline(example_data.true_mi, color='red', linestyle='-', linewidth=2, label='True MI')
