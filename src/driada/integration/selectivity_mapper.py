...
class SelectivityManifoldMapper:
    ...
    def __init__(
        ...
        self.has_selectivity = hasattr(experiment, "stats_tables") and bool(
            experiment.stats_tables
        )
    ...
    def create_embedding(
        ...
        **dr_kwargs
        ...
        if ds > 1:
            ...
            if self.logger:
                self.logger.info(
                    f"Downsampling data by factor {ds}: {neural_data.shape[1]} timepoints"
                )
    ...
    def get_functional_organization(
        self, method_name: str, data_type: str = "calcium"
    ) -> Dict:
        """
        ...
        neuron_indices = metadata.get(
            "neuron_indices", list(range(self.experiment.n_cells))
        )
        ...
        has_embedding_selectivity = (
            hasattr(self.experiment, "stats_tables")
            and data_type in self.experiment.stats_tables
            and stats_key in self.experiment.stats_tables[data_type]
        )
        ...
        if has_embedding_selectivity:
            ...
            for comp_idx in range(embedding.shape[1]):
                ...
                for neuron_idx in range(self.experiment.n_cells):
                    if (
                        feat_name in self.experiment.significance_tables[data_type]
                        and neuron_idx
                        in self.experiment.significance_tables[data_type][feat_name]
                        and self.experiment.significance_tables[data_type][feat_name][
                            neuron_idx
                        ].get("stage2", False)
                    ):
                ...
                component_specialization[comp_idx] = {
                    ...
                    "selectivity_rate": len(selective_neurons)
                    / self.experiment.n_cells,
                }
    ...
    def compare_embeddings(
        self, method_names: List[str], data_type: str = "calcium"
    ) -> Dict:
        """
        ...
        for method in method_names:
            try:
                organizations[method] = self.get_functional_organization(
                    method, data_type
                )
            except KeyError:
