...
def main(quick_test=False, seed=42, enable_visualizations=True, 
         filter_method='none', filter_params=None, include_noisy=False):
    ...
    print(f"  Ground truth spatial neurons: {n_true_spatial} (indices 0-{n_true_spatial-1})")
    print(f"  Ground truth non-spatial neurons: {exp.n_cells - n_true_spatial} (indices {n_true_spatial}-{exp.n_cells-1})")
    ...
    precision = len(true_positives) / len(detected_spatial_set) if detected_spatial_set else 0
    ...
    r2_true_spatial = (r2_score(y_test[:, 0], y_pred_true[:, 0]) + r2_score(y_test[:, 1], y_pred_true[:, 1])) / 2
    ...
    r2_true_nonspatial = (r2_score(y_test[:, 0], y_pred_nonspatial[:, 0]) + r2_score(y_test[:, 1], y_pred_nonspatial[:, 1])) / 2
    ...
    print(f"  Spatial vs Non-spatial ratio: {r2_true_spatial/max(r2_true_nonspatial, 0.001):.2f}x")
    ...
    for feat in ['d_feat_0', 'd_feat_1', 'd_feat_2', 'c_feat_0', 'c_feat_1', 'c_feat_2']:
        try:
    ...
    calcium_all_noisy = calcium_all + np.random.normal(0, noise_level, calcium_all.shape)
    calcium_spatial_noisy = calcium_spatial + np.random.normal(0, noise_level, calcium_spatial.shape)
    ...
    for method_name, method_config in dr_methods.items():
        ...
        for scenario_name, calcium_data in scenarios:
            ...
            try:
                ...
                # Compute metrics
                metrics = compute_spatial_correspondence_metrics(embedding, true_positions)
                ...
                # Print key metrics
                print(f"      Spatial decoding R²: {metrics['spatial_decoding_r2_avg']:.3f}, "
        ...
        if "All neurons" in results[method_name] and "Spatial neurons" in results[method_name]:
            if results[method_name]["All neurons"] and results[method_name]["Spatial neurons"]:
                r2_all = results[method_name]["All neurons"]['metrics']['spatial_decoding_r2_avg']
                r2_spatial = results[method_name]["Spatial neurons"]['metrics']['spatial_decoding_r2_avg']
                improvement = (r2_spatial/max(r2_all, 0.001)-1)*100
                print(f"    Spatial vs All improvement: {improvement:+.1f}%")
        
        if "All neurons" in results[method_name] and "Random half" in results[method_name]:
            if results[method_name]["All neurons"] and results[method_name]["Random half"]:
                r2_all = results[method_name]["All neurons"]['metrics']['spatial_decoding_r2_avg']
                r2_random = results[method_name]["Random half"]['metrics']['spatial_decoding_r2_avg']
    ...
    if enable_visualizations:
        ...
        for idx, (metric_key, metric_title) in enumerate(metrics_to_show):
            ...
            ax.set_xticks(x)
            ax.set_xticklabels([label for _, label in scenarios_to_show], rotation=45, ha='right')
    ...
    for method_name in results.keys():
        if "Spatial neurons" in results[method_name] and results[method_name]["Spatial neurons"]:
            score = results[method_name]["Spatial neurons"]['metrics']['spatial_decoding_r2_avg']
            if score > best_score:
                best_score = score
    ...
    for method_name in results.keys():
        print(f"\n  {method_name}:")
        scenarios_order = ["All neurons", "Spatial neurons", "Random half", "Non-selective"]
        ...
        if "All neurons" in results[method_name] and results[method_name]["All neurons"]:
            r2_all = results[method_name]["All neurons"]['metrics']['spatial_decoding_r2_avg']
            ...
            if "Random half" in results[method_name] and results[method_name]["Random half"]:
                r2_random = results[method_name]["Random half"]['metrics']['spatial_decoding_r2_avg']
                ratio = r2_random / max(r2_all, 0.001)
    ...
    if include_noisy:
        ...
        for method_name in results.keys():
            if "All neurons (noisy)" in results[method_name] and "Spatial neurons (noisy)" in results[method_name]:
                if results[method_name]["All neurons (noisy)"] and results[method_name]["Spatial neurons (noisy)"]:
                    r2_all = results[method_name]["All neurons (noisy)"]['metrics']['spatial_decoding_r2_avg']
                    r2_spatial = results[method_name]["Spatial neurons (noisy)"]['metrics']['spatial_decoding_r2_avg']
                    imp = (r2_spatial / max(r2_all, 0.001) - 1) * 100
                    print(f"  {method_name}: All {r2_all:.3f} → Spatial {r2_spatial:.3f} ({imp:+.1f}%)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='INTENSE-guided dimensionality reduction pipeline')
    ...
    parser.add_argument('--filter-snr', type=float, default=2.0,
    ...
    if args.filter != 'none':
        if args.filter == 'gaussian':
            ...
            filter_params = {'window_length': args.filter_window, 'polyorder': args.filter_polyorder}
        elif args.filter == 'adaptive':
            filter_params = {'snr_threshold': args.filter_snr}
    
    exp, results = main(quick_test=args.quick, seed=args.seed,
