...
def visualize_spatial_analysis(n_samples=2000, n_neurons=4, seed=42):
    ...
    im = ax.imshow(occupancy_map, origin='lower', cmap='Blues',
    ...
    for i in range(n_neurons):
        ...
        scatter = ax.scatter(positions[:, 0], positions[:, 1], 
                           c=calcium_signals[i], s=1, 
                           cmap='hot', alpha=0.5)
        ax.plot(centers[i, 0], centers[i, 1], 'w*', markersize=10, 
        ...
        im = ax.imshow(rate_map, origin='lower', cmap='hot',
                       extent=[0, 1, 0, 1], aspect='equal')
        ...
        ax.set_title(f'Calcium Trace (first 25s)')
...
def main():
    ...
    fig = visualize_spatial_analysis(
        n_samples=2000,
        n_neurons=4,
        seed=42
    )
    ...
    calcium_signals = np.array([
        generate_pseudo_calcium_signal(fr, sampling_rate=20.0)
    ...
    print(f"  Mean spatial information: {results['summary']['mean_spatial_info']:.3f} bits/spike")
