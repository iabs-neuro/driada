...
def main():
    ...
    spikes_threshold, meta_threshold = reconstruct_spikes(
        calcium, method='threshold', fps=fps,
    ...
    for key, value in meta_wavelet['parameters'].items():
        if isinstance(value, (int, float, str)):
            print(f"  {key}: {value}")
    print(f"Total events detected: {sum(len(events) for events in meta_wavelet['start_events'])}")
    ...
    for key, value in meta_threshold['parameters'].items():
        print(f"  {key}: {value}")
    print(f"Total spikes detected: {sum(len(times) for times in meta_threshold['spike_times'])}")
