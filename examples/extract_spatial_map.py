...
from driada.dimensionality import (
    pca_dimension, effective_rank, nn_dimension, correlation_dimension
)
from driada.dim_reduction.manifold_metrics import (
    knn_preservation_rate, trustworthiness, continuity,
    geodesic_distance_correlation, procrustes_analysis
)
...
def visualize_place_fields(centers, field_sigma):
    ...
    for center in centers[:20]:  # Show first 20 to avoid clutter
        circle = plt.Circle(center, field_sigma, fill=False, 
...
def apply_dimensionality_reduction(neural_data, true_positions, n_neighbors=30):
    ...
    metrics['Isomap'] = {
        'knn_preservation': knn_preservation_rate(true_positions, isomap_embedding, k=10),
...
def test_noise_robustness(neural_data, true_positions, noise_levels):
    ...
    methods = ['PCA', 'Isomap']
    robustness_results = {method: {'noise_levels': noise_levels,
                                  'knn_preservation': [],
                                  'procrustes_error': []} 
    ...
    for noise_std in noise_levels:
        ...
        isomap_metrics = {
            'knn_preservation': knn_preservation_rate(true_positions, isomap_embedding, k=10),
            'procrustes_error': procrustes_analysis(true_positions, isomap_embedding)[1]
        }
        
        # Store results
        robustness_results['PCA']['knn_preservation'].append(pca_metrics['knn_preservation'])
        robustness_results['PCA']['procrustes_error'].append(pca_metrics['procrustes_error'])
        robustness_results['Isomap']['knn_preservation'].append(isomap_metrics['knn_preservation'])
        robustness_results['Isomap']['procrustes_error'].append(isomap_metrics['procrustes_error'])
...
def plot_noise_robustness(robustness_results):
    ...
    for method, results in robustness_results.items():
        # k-NN preservation
        ax1.plot(results['noise_levels'], results['knn_preservation'], 
                'o-', label=method, linewidth=2, markersize=8)
        
        # Procrustes error
        if results['procrustes_error']:
            ax2.plot(results['noise_levels'], results['procrustes_error'], 
                    'o-', label=method, linewidth=2, markersize=8)
...
def main():
    ...
    print(f"{'Method':10s} | {'k-NN Pres':10s} | {'Trust':10s} | {'Cont':10s} | {'Procrustes':10s}")
    print("-" * 60)
    
    for method in ['PCA', 'Isomap', 'UMAP']:
        m = metrics[method]
        print(f"{method:10s} | {m['knn_preservation']:10.3f} | "
