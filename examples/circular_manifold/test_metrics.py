...
print(f"Extracted angles range: [{extracted_angles.min():.2f}, {extracted_angles.max():.2f}]")
...
# Check for sign flip
if np.corrcoef(true_vel, -extracted_vel)[0,1] > np.corrcoef(true_vel, extracted_vel)[0,1]:
    print("\nWARNING: Velocities are anti-correlated - possible sign flip!")
    print(f"Correlation with flipped sign: {np.corrcoef(true_vel, -extracted_vel)[0,1]:.3f}")