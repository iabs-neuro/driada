...
from driada.dim_reduction import (
    ...
    stress, procrustes_analysis, manifold_preservation_score
)
...
def generate_test_datasets(n_samples: int = 1000, noise: float = 0.0, seed: int = 42) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    """
    ...
    X_swiss, color_swiss = make_swiss_roll(n_samples=n_samples, noise=noise, random_state=seed)
    datasets['swiss_roll'] = (X_swiss, color_swiss)
    
    # 2. S-Curve - another nonlinear manifold
    print("Generating S-Curve...")
    X_scurve, color_scurve = make_s_curve(n_samples=n_samples, noise=noise, random_state=seed)
...
def evaluate_dr_method(
    ...
    try:
        ...
        if coords.shape[0] == data_filtered.shape[0]:
            ...
            results['error'] = f"Dimension mismatch: {coords.shape[0]} vs {data_filtered.shape[0]}"
...
def run_comparison(
    ...
    for dataset_name, (data, labels) in datasets.items():
        ...
        for method_name, method_config in methods.items():
            current += 1
            print(f"\n[{current}/{total_comparisons}] Evaluating {method_name}...", end='', flush=True)
...
def visualize_comparison_results(results_df: pd.DataFrame, save_prefix: str = 'dr_comparison'):
    """
    ...
    success_df['avg_quality'] = success_df[['knn_preservation', 'trustworthiness', 'continuity']].mean(axis=1)
    ...
    if len(swiss_results) > 0:
        ...
        plt.tight_layout()
        plt.savefig(f'{save_prefix}_swiss_roll_embeddings.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def generate_recommendations(results_df: pd.DataFrame) -> Dict[str, str]:
    ...
    method_summary = success_df.groupby('method').agg({
