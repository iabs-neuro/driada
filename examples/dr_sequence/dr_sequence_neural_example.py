...
def main():
    ...
    embedding_direct = mvdata.get_embedding(
        method='le',
        dim=2,
        n_neighbors=20
    )
    ...
    metrics_direct = compute_manifold_metrics(
        true_positions_ds,
        embedding_direct.coords
    )
    
    metrics_sequence = compute_manifold_metrics(
        true_positions_ds,
        embedding_sequence.coords
    )
    ...
    for metric in ['knn_preservation', 'trustworthiness', 'continuity']:
        improvement = (metrics_sequence[metric] - metrics_direct[metric]) / metrics_direct[metric] * 100
    ...
    fig_comparison = plot_embedding_comparison(
        ...
        feature_names={'time': 'Time', 'x_position': 'X Position', 'y_position': 'Y Position'},
