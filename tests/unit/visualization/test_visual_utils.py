...
class TestVisualUtils:
    ...
    def sample_embeddings(self):
        ...
        embeddings = {
            'pca': np.column_stack([np.cos(angles), np.sin(angles)]) + 0.1 * np.random.randn(n_samples, 2),
            'umap': np.column_stack([1.5*np.cos(angles), 1.5*np.sin(angles)]) + 0.15 * np.random.randn(n_samples, 2),
            'le': np.column_stack([0.8*np.cos(angles), 0.8*np.sin(angles)]) + 0.1 * np.random.randn(n_samples, 2)
        }
    ...
    def sample_metrics_data(self):
        ...
        data = {
            'pca': {
                'all_neurons': {'spatial_decoding_r2': 0.85, 'distance_correlation': 0.78, 'runtime': 0.5},
                'spatial_neurons': {'spatial_decoding_r2': 0.92, 'distance_correlation': 0.88, 'runtime': 0.3},
                'random_half': {'spatial_decoding_r2': 0.82, 'distance_correlation': 0.75, 'runtime': 0.25}
            },
            'umap': {
                'all_neurons': {'spatial_decoding_r2': 0.88, 'distance_correlation': 0.82, 'runtime': 2.5},
                'spatial_neurons': {'spatial_decoding_r2': 0.94, 'distance_correlation': 0.91, 'runtime': 1.8},
                'random_half': {'spatial_decoding_r2': 0.85, 'distance_correlation': 0.80, 'runtime': 1.2}
            }
        }
        return data
    ...
    def test_plot_component_interpretation(self):
        ...
        metadata = {
            'pca': {
                'explained_variance_ratio': np.array([0.4, 0.25, 0.15, 0.1, 0.05])
            }
        }
    ...
    def test_plot_embeddings_grid(self, sample_embeddings, sample_metrics_data):
        ...
        grid_embeddings = {
            'pca': {
                'all': embeddings['pca'],
                'subset': embeddings['pca'][:100]
            },
            'umap': {
                'all': embeddings['umap'],
                'subset': embeddings['umap'][:100]
            }
        }
