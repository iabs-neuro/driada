...
from driada.experiment.exp_base import (
    Experiment, check_dynamic_features, STATS_VARS, SIGNIFICANCE_VARS,
    DEFAULT_STATS, DEFAULT_SIGNIFICANCE
)
...
class TestExperimentInitialization:
    ...
    def test_feature_attribute_conflict(self):
        ...
        with pytest.raises(ValueError, match="Dynamic feature names conflict with protected attributes.*neurons"):
            Experiment(
                signature="test",
                calcium=calcium,
                spikes=None,
    ...
    def test_static_feature_conflict(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert any("conflicts with protected attribute" in str(warning.message) for warning in w)
...
class TestExperimentMethods:
    ...
    def test_embedding_shape_validation(self, basic_experiment):
        ...
        basic_experiment.store_embedding(
            embedding, 'pca_ds2', metadata={'ds': ds}
        )
