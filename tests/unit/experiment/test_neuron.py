...
from driada.experiment.neuron import Neuron, DEFAULT_T_RISE, DEFAULT_T_OFF, DEFAULT_FPS, MIN_CA_SHIFT
...
class TestNeuronInitialization:
    ...
    def test_init_with_custom_parameters(self):
        ...
        neuron = Neuron("cell_3", ca_data, sp_data, 
                       default_t_rise=0.5, default_t_off=3.0, fps=30.0)
...
class TestNeuronMethods:
    ...
    def test_reconstruct_spikes_not_implemented(self):
        ...
        with pytest.raises(AttributeError, match="Spike reconstruction not implemented"):
...
class TestCalciumShuffling:
    ...
    def test_shuffle_calcium_roll_based(self):
        ...
        assert np.allclose(np.sort(shuffled), np.sort(neuron.ca.data), atol=1e-6)  # Same values, different order
    ...
    def test_shuffle_calcium_roll_based_with_shift(self):
        ...
        shift = 100
        shuffled = neuron.get_shuffled_calcium(method='roll_based', no_ts=True, shift=shift)
    ...
    def test_shuffle_calcium_waveform_based(self):
        ...
        with patch.object(neuron, '_shuffle_spikes_data_isi_based', 
                         return_value=np.roll(sp_data, 50)):
...
class TestIntegration:
    ...
    def test_wavelet_spike_inference(self):
        ...
        pseudo_calcium = generate_pseudo_calcium_multisignal(
            n, duration, sampling_rate, event_rate,
            amplitude_range, decay_time, noise_std
        )
