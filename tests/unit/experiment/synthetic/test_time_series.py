...
class TestBinaryTimeSeries:
    ...
    def test_generate_binary_time_series_basic(self):
        ...
        assert 0.5 * avg_islands <= n_islands <= 1.5 * avg_islands, \
        ...
        if n_islands > 0:
            ...
            assert 0.5 * avg_duration <= mean_duration <= 1.5 * avg_duration, \
        ...
        assert 0.5 * expected_fraction <= active_fraction <= 1.5 * expected_fraction, \
    ...
    def test_realistic_parameters(self):
        ...
        assert 0.001 <= active_fraction <= 0.3, \
